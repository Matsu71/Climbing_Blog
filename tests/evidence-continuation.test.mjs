...
test('continuation preserves previous claims and records explicit additions',()=>{
 ...
 assert.deepEqual(db.grade_claims.slice(0,initial.grade_claims.length),initial.grade_claims);assert.equal(db.queue.length,21);
});
...
test('record filters and access status are rendered in static HTML',()=>{
 ...
 const source=renderEvidence('evidence','/');assert.match(source,/data-results/);assert.match(source,/一部採録/);assert.match(source,/収録出典67件/);
});
