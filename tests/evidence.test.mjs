...
test('coverage counts do not count missing, ambiguous or calculated values as source measurements',()=>{
 assert.deepEqual([evidenceCounts.athletes,evidenceCounts.measurements,evidenceCounts.ascents,evidenceCounts.clinicalStudies,evidenceCounts.clinicalSources,evidenceCounts.sources],[30,54,64,10,12,67]);
 assert.equal(db.collection_complete,false);assert.equal(db.expert_reviewed,false);
 assert.equal(db.measurements.length,56);assert.equal(db.athletes.length,39);
});
...
test('data pages are indexable HTML with single H1, real sources and bounded labels',()=>{
 ...
 const h=render('athlete-data');assert.equal((h.match(/id="athlete-/g)||[]).length,30);assert.match(h,/169/);assert.match(h,/170/);assert.match(h,/測定時期/);
 assert.equal((render('ascent-data').match(/id="ascent-/g)||[]).length,64);
 assert.equal((render('evidence').match(/id="source-/g)||[]).length,67);
...
test('branch-based Pages mirrors use the same audited code and data',()=>{
 for(const path of ['bootstrap.mjs','single-article.mjs','src/render.mjs','src/knowledge-render.mjs','src/evidence-render.mjs','data/evidence.mjs','data/evidence-initial.mjs','data/evidence-additions.mjs','data/evidence-expansion.mjs','src/core.mjs','core.mjs','evidence.css','evidence-ui.mjs','index.html'])assert.equal(readFileSync(path,'utf8'),readFileSync('docs/'+path,'utf8'),path);
