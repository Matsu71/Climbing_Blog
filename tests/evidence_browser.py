...
def evidence_checks(page,go,run,ROOT,OUT):
    def body_data():
        ...
        assert page.locator('tbody [data-item]:visible').count()==7
        ...
        assert page.locator('tbody [data-item]:visible').count()==24
    ...
    def ascent_download():
        go('ascent-data/')
        assert page.locator('tbody [data-item]').count()==41
    ...
    def medical():
        go('injury-data/')
        assert page.locator('.study-entry').count()==8
    ...
    def filtered_anchor():
        ...
        page.locator('[name=access]').select_option('抄録')
        assert page.locator('.source-register [data-item]:visible').count()==2
        ...
        page.locator('button[type=reset]').click()
        assert page.locator('.source-register [data-item]:visible').count()==53
    ...
    def data_screenshots():
        ...
        cases=[('body','athlete-data/?q=Laura'),('ascents','ascent-data/?q=Bibliographie'),('injury','injury-data/?condition='+quote('プーリー')),('sources','evidence/?q=EDELRID')]
        try:
