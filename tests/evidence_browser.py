...
def evidence_checks(page,go,run,ROOT,OUT):
    def body_data():
        ...
        assert page.locator('tbody [data-item]:visible').count()==17
    ...
    def athlete_style():
        ...
        expect(page.locator('#ascent-continental-laura')).to_contain_text('資料公開 2026-05-05')
    ...
    def applicability():
        ...
        expect(page.locator('.study-entry:visible')).to_contain_text('340人')
    ...
    def filtered_exports():
        go('athlete-data/?q=Nolwen')
        expect(page.locator('#athlete-nolwen-berthier')).to_contain_text('表記差あり')
        page.locator('[data-export-scope]').select_option('visible')
        expect(page.locator('[data-export-note]')).to_contain_text('選手の全項目')
        with page.expect_download() as event:
            page.locator('[data-evidence-download="evidence-measurements.json"]').click()
        dest=OUT/'selected-body.json';event.value.save_as(str(dest));bundle=json.loads(dest.read_text())
        assert event.value.suggested_filename=='evidence-measurements-filtered.json'
        assert bundle['scope']=='visible' and bundle['record_count']==3
        assert [r['value'] for r in bundle['rows'] if r['metric']=='height']==[156,152]
        assert len(bundle['sources'])==1 and bundle['filters']['q']=='Nolwen'
        go('ascent-data/?climb=Box%20Therapy')
        page.locator('[data-export-scope]').select_option('visible')
        with page.expect_download() as event:
            page.locator('[data-evidence-download="evidence-ascents.json"]').click()
        dest=OUT/'selected-ascents.json';event.value.save_as(str(dest));bundle=json.loads(dest.read_text())
        assert bundle['record_count']==3 and len(bundle['athletes'])==3
        assert len(bundle['grade_claims'])==1
        with page.expect_download() as event:
            page.locator('[data-evidence-download="evidence-ascents.csv"]').click()
        dest=OUT/'selected-ascents.csv';event.value.save_as(str(dest))
        with dest.open(encoding='utf-8-sig',newline='') as handle: rows=list(csv.DictReader(handle))
        assert len(rows)==3 and all(r['source_url'].startswith('https:') for r in rows)
        assert all(json.loads(r['export_filters'])['climb']=='Box Therapy' for r in rows)
        page.locator('[name=q]').fill('unmatched0000000')
        expect(page.locator('[data-evidence-download="evidence-ascents.json"]')).to_be_disabled()
        page.locator('[data-export-scope]').select_option('all')
        expect(page.locator('[data-evidence-download="evidence-ascents.json"]')).to_be_enabled()
    run('evidence: filtered body and ascent exports retain provenance, conflicts and empty-state safety',filtered_exports)

    def reviewed_export():
        go('injury-data/?purpose='+quote('再発予防'))
        expect(page.locator('.study-entry:visible .access-badge')).to_have_text('本文確認')
        page.locator('[data-export-scope]').select_option('visible')
        with page.expect_download() as event:
            page.locator('[data-evidence-download="evidence-clinical.json"]').click()
        dest=OUT/'selected-clinical.json';event.value.save_as(str(dest));bundle=json.loads(dest.read_text())
        assert bundle['record_count']==1 and len(bundle['revision_history'])==1
        before=bundle['revision_history'][0]['before']; after=bundle['rows'][0]
        assert before['access']=='抄録' and after['access']=='本文'
        assert after['analysis_counts']==[107,113,120] and before['outcome_counts']==[]
        assert len(bundle['sources'])==2
        go('injury-data/?condition='+quote('指の屈筋腱鞘炎'))
        assert page.locator('.study-entry:visible').count()==2
        expect(page.locator('#study-tenosynovitis-injection')).to_contain_text('長期安全性')
    run('evidence: full-text review exports keep earlier claims and separate noncomparative treatments',reviewed_export)

    def date_precision():
        go('ascent-data/?climb=Kryptos&precision='+quote('月まで'))
        expect(page.locator('.filter-extra')).to_have_attribute('open','')
        expect(page.locator('tbody [data-item]:visible')).to_have_attribute('id','ascent-kryptos-kaddi')
        expect(page.locator('#ascent-kryptos-kaddi time')).to_have_text('2018-05')
    ...
    def data_screenshots():
        ...
        cases=[('nolwen','athlete-data/?q=Nolwen'),('tenosynovitis','injury-data/?condition='+quote('指の屈筋腱鞘炎')),('body','athlete-data/?q=Laura'),('ascents','ascent-data/?q=Bibliographie'),('injury','injury-data/?condition='+quote('プーリー')),('sources','evidence/?q=EDELRID'),('box','ascent-data/?climb=Box%20Therapy'),('ankle','injury-data/?scope='+quote('一般スポーツ'))]
        try:
