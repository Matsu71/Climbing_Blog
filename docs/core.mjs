...
export function csv(rows,fields){const cell=v=>{let t=v==null?'':Array.isArray(v)?v.join(' | '):String(v);if(/^[=+\-@\t\r]/.test(t))t="'"+t;return '"'+t.replace(/"/g,'""')+'"';};return '\ufeff'+[fields,...rows.map(r=>fields.map(f=>r[f]))].map(r=>r.map(cell).join(',')).join('\r\n')+'\r\n';}
