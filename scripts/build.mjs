...
for(const name of ['core','gym-model','notebook-model'])await copyFile(resolve(root,'src/'+name+'.mjs'),resolve(dist,name+'.mjs'));
...
const routes=[['home',null],['read',null],...articles.map(a=>['read',a.id]),['climbs',null],...climbs.map(c=>['climbs',c.id]),['athletes',null],...athletes.map(a=>['athletes',a.id]),['research',null],['gyms',null],...gyms.map(g=>['gyms',g.id]),...['glossary','notebook','gym-history','tools','quiz','search','saved','quality','data','privacy','404'].map(v=>[v,null])];
const paths=[];
