...
await copyFile(resolve(root,'src/core.mjs'),resolve(dist,'core.mjs'));
...
const routes=[['home',null],['read',null],...articles.map(a=>['read',a.id]),['climbs',null],...climbs.map(c=>['climbs',c.id]),['athletes',null],...athletes.map(a=>['athletes',a.id]),['research',null],['gyms',null],...gyms.map(g=>['gyms',g.id]),...['tools','quiz','search','saved','quality','data','privacy','404'].map(v=>[v,null])];
const paths=[];
...
for(const [name,data]of Object.entries({sources,climbs,athletes,studies,gyms,taxonomy,articles,quiz,search:searchIndex()}))await writeFile(resolve(dist,'data',name+'.json'),JSON.stringify(data,null,2)+'\n');
for(const [name,data,fields]of [['climbs',climbs,['id','name','discipline','country','area','first_ascent_by','first_ascent_date','date_precision','grade_at_source','current_consensus','repeat_count','sources','checked_at']],['gyms',gyms,['id','name','prefecture','municipality','address','disciplines','sources','checked_at']],['studies',studies,['id','title','year','design','theme','participants','protocol','finding','limit','sources']]])await writeFile(resolve(dist,'data',name+'.csv'),csv(data,fields));
...
await writeFile(resolve(dist,'build-manifest.json'),JSON.stringify({base,origin:originURL.origin,pages:routes.length,paths,counts:{articles:articles.length,climbs:climbs.length,athletes:athletes.length,studies:studies.length,gyms:gyms.length,sources:sources.length},editorial_date:checkedAt},null,2)+'\n');
